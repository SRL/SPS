...
uses
  Classes, Bitmaps, MufasaTypes, spsTypes;

function SPS_FindMapInMap(out fx, fy: integer; LargeMap, SmallMap: T3DIntegerArray; tol: extended): integer; cdecl;
function SPS_ColorBoxesMatchInline(B1, B2: TIntegerArray; tol: extended): boolean; inline;
procedure SPS_MakeColorBox(bmp: TMufasaBitmap; x1, y1, SideLength: integer; var res: TIntegerArray); cdecl;
procedure SPS_BitmapToMap(bmp: TMufasaBitmap; SideLength: integer; var res: T3DIntegerArray); cdecl;
function SPS_FindMapInMapArray(LargeMap, SmallMap: T3DIntegerArray; tol: extended): T2DSingleArray; cdecl;
...
begin
  ...
  for x := (x1 + SideLength - 1) downto x1 do
    ...
    begin
      ...
      Res[0] := Res[0] + bmp.FData[c].R;
      Res[1] := Res[1] + bmp.FData[c].G;
      Res[2] := Res[2] + bmp.FData[c].B;
    end;
end;
...
begin
  ...
  SetLength(Result, HighY + 1, HighX + 1);
